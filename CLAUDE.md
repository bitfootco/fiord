...
├── scripts/
│   └── generate.js              ← Builds public/*/components/ from src/ definitions
├── src/                         ← Authored source — human-readable pages + component definitions
│   ├── layouts/
...
│   │       └── pumice/
│   │           └── index.astro  ← Pumice aesthetic showcase
│   ├── base/
│   │   ├── boilerplate.json     ← Base page chrome config (fonts, body class, wrapper class)
│   │   └── components/          ← Base component definition files (42 HTML + JS — AUTHORED HERE)
│   └── aesthetics/
│       ├── slate/
│       │   ├── boilerplate.json ← Slate page chrome config
│       │   └── components/      ← Slate component definition files (AUTHORED HERE)
│       └── pumice/
│           ├── boilerplate.json ← Pumice page chrome config
│           └── components/      ← Pumice component definition files (AUTHORED HERE)
├── public/                      ← Served verbatim — agents scrape here
│   ├── manifest.json            ← Machine-readable discovery document (AUTHORED)
│   ├── manifest.md              ← Human-readable discovery document (AUTHORED)
│   ├── assets/                  ← Static assets: logos, etc. (AUTHORED)
│   ├── skills/                  ← Agent skill files (AUTHORED)
│   ├── base/
│   │   └── components/          ← GENERATED by scripts/generate.js — do not edit
│   └── aesthetics/
│       ├── slate/
│       │   ├── meta.json        ← Slate design language spec (AUTHORED)
│       │   └── components/      ← GENERATED by scripts/generate.js — do not edit
│       └── pumice/
│           ├── meta.json        ← Pumice design language spec (AUTHORED)
│           └── components/      ← GENERATED by scripts/generate.js — do not edit
...
The component files in `public/*/components/` are **generated** — they are written by `scripts/generate.js` and committed to the repo so they are deployable without a build step. They are gitignored from the components subdirectories only; `meta.json`, `manifest.json`, `manifest.md`, and other hand-authored files in `public/` are committed normally. Never edit generated files directly — edit the definition files in `src/` and re-run the generator.
...
```
npm run build     → runs generate first (prebuild), then Astro → dist/
npm run generate  → rebuild public/*/components/ from src/ definitions only
...
1. Create `src/aesthetics/<name>/boilerplate.json` — page chrome config (fonts, Tailwind config, body/wrapper classes)
2. Create `public/aesthetics/<name>/meta.json` — full design language spec (palette, typography, spacing, borders, shadows, motion)
3. Write 42 HTML definition files in `src/aesthetics/<name>/components/` — pure component markup, no boilerplate (see Component Authoring Workflow below)
4. Write JS definition files for interactive components (dropdown, tabs, modal, drawer, tooltip, popover, toast, faq) in the same directory
5. Run `npm run generate` → produces `public/aesthetics/<name>/components/` (42 full-page HTML files, never hand-edited)
6. Create `src/pages/aesthetics/<name>/index.astro` as a catalog/showcase page using `SiteLayout`
7. Add the aesthetic to both `manifest.md` and `manifest.json`
